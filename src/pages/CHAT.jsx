...
const CHAT = () => {
    ...
    const loadChatHistory = async () => {
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          if (lastMsg?.batchId) {
            setBatchId(Number(lastMsg.batchId));
          }
        } else {
          setMessages([
            {
  ...
  const suggestionButtons = [
    {
      text: "광고 제작",
      response:
        "Adwise 광고 제작은 업종과 타겟 정보를 입력하면, AI가 자동으로 문구와 이미지를 생성해 각 플랫폼에 맞게 최적화해드립니다.",
    },
    {
      text: "제작 기간",
    ...
    { text: "수수료 문의", response: "수수료는 광고 예산의 10%입니다." },
    {
      text: "차트 오류",
      response:
        "차트 오류가 발생했다면, 새로고침하거나 잠시 후 다시 시도해주세요.",
    },
    {
      text: "기타 문의사항",
      response: "기타 문의는 support@adwise.com 으로 연락주세요.",
    },
  ];
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      // payload에 batchId는 유효할 때만 추가
      const payload = { message: trimmedInput };
      if (!isNaN(Number(batchId)) && Number(batchId) > 0) {
      ...
      if (data.batchId) {
        setBatchId(Number(data.batchId));
      }

      setMessages((prev) =>
        prev.filter((msg) => msg.id !== "thinking").concat(assistantMessage)
      );
    } catch (error) {
      ...
      <div className="flex items-center justify-center min-h-screen bg-[#1B1B1B] text-white font-sans">
        <div className="w-full max-w-5xl bg-[#1B1B1B] border border-orange-500 rounded-xl p-7 flex flex-col mx-4 mt-24 mb-48 h-[90vh] max-h-[800px]">
              ...
              >
                <div
                  className={`py-3 px-5 rounded-2xl max-w-[80%] ${
                    msg.sender === "user"
                      ? "bg-zinc-600"
                      : "bg-[#FF9550] text-zinc-800 font-bold"
                  }`}
                >
                  {msg.thinking ? <ThinkingIndicator /> : <p>{msg.text}</p>}
                </div>
              </div>
            ))}
          </div>
          ...
          <form
            ...
            className="flex items-center bg-zinc-700 rounded-full p-2 flex-shrink-0 mt-6"
          >
            <input
              type="text"
              value={inputValue}
              ...
              placeholder={
                isLoading ? "응답을 기다리는 중..." : "채팅을 입력하세요."
              }
